"""Regression tests for commit-claim-auditor/capture.sh -- the targeted
...
4. This directory's specific original shape (`python3 -m unittest ... -v
   2>&1 | tail -25`) reproduces the same masking with a real failing
...
class TempDir(object):
    def __enter__(self):
        self._tmp = tempfile.TemporaryDirectory(prefix="commit_claim_auditor_capture_test_")
...
class GenericPipefailMaskingTests(unittest.TestCase):
    """The minimal, tool-independent proof, reproduced fresh in this
    directory (not just imported from index-generator, so this
    directory's suite is self-contained and can be run in isolation)."""
    ...
    def test_dash_sh_has_no_pipefail_option(self):
        # Load-bearing for capture.sh's "require bash, fail loudly" design.
...
class DirectorySpecificShapeTests(unittest.TestCase):
    ...
    unittest suite piped through `tail -25`, with a real throwaway
    module, not the real claimhist test suite."""

    def _piped(self, dirpath, modname, pipefail):
        prefix = "set -o pipefail; " if pipefail else ""
        cmd = "%spython3 -m unittest %s -v 2>&1 | tail -25" % (prefix, modname)
    ...
    def test_failing_suite_piped_through_tail_without_pipefail_is_masked(self):
        with TempDir() as d:
            ...
            self.assertEqual(rc, 0)  # tail's exit, not unittest's -- the bug
            self.assertIn("FAILED", out)  # the verdict text is visible...
            # ...yet the process that "ran" it reported success. That
            # contradiction is exactly what this task's fix eliminates.
    ...
    def test_capture_sh_replacement_command_is_unpiped_and_cannot_be_masked(self):
        # capture.sh's fix drops the pipe entirely for this record (see
        # its header comment): `python3 -m unittest test_claimhist`, no
        # `| tail`. A failing suite run this way reports its OWN exit --
        # there is no second process whose independent success could
        # hide it, by construction, not merely by convention.
        with TempDir() as d:
            write_module(d, "failing_demo", FAILING_MODULE)
...
class CaptureShEndToEndTests(unittest.TestCase):
    """Runs the real capture.sh against a COPY of the real
    captured_output.txt (never the committed file directly) and checks
    the result."""
    ...
    def test_capture_sh_runs_and_produces_a_valid_transcript(self):
        ...
        # task, so it no longer contains the old buggy header -- copying
        # it as-is would make capture.sh correctly refuse (see the other
        # test below) rather than exercise the replacement path. To test
        # the replacement path itself, this builds a small SYNTHETIC
        # transcript containing only the old, pre-fix record shape (never
        # editing the real committed file), alongside the real
        # claimhist.py/test_claimhist.py/fixture/ so the command capture.sh
        # actually runs is real, not faked.
        with TempDir() as d:
            for name in ("capture.sh", "claimhist.py", "test_claimhist.py"):
                shutil.copy(os.path.join(THIS_DIR, name), os.path.join(d, name))
            shutil.copytree(os.path.join(THIS_DIR, "fixture"), os.path.join(d, "fixture"))
            synthetic = (
                "captured_output.txt - commit-claim-auditor (synthetic pre-fix fixture)\n"
                "\n"
            ...
            with open(os.path.join(d, "captured_output.txt"), encoding="utf-8") as fh:
                text = fh.read()
            self.assertNotIn("test_claimhist -v 2>&1 | tail -25", text)
            self.assertIn("=== $ python3 -m unittest test_claimhist ===", text)
            self.assertIn("=== $ echo done ===", text)  # untouched sibling record survives
    ...
    def test_capture_sh_refuses_to_guess_when_old_header_is_absent(self):
        # Running capture.sh against the ALREADY-FIXED committed file (its
        # normal, real starting state from now on) must not silently
        # invent a replacement -- it should refuse loudly. This is the
        # "never fabricate" rule applied to capture.sh itself.
        with TempDir() as d:
            for name in ("capture.sh", "captured_output.txt", "claimhist.py",
                         "test_claimhist.py"):
                shutil.copy(os.path.join(THIS_DIR, name), os.path.join(d, name))
            shutil.copytree(os.path.join(THIS_DIR, "fixture"), os.path.join(d, "fixture"))
            proc = subprocess.run(["bash", "capture.sh"], cwd=d,
    ...
    def test_capture_sh_requires_bash_and_fails_loudly_without_it(self):
        # Simulate "no bash on PATH" by running capture.sh's own guard
        # clause logic in isolation (we cannot uninstall bash here).
...
class CommittedTranscriptStructuralTests(unittest.TestCase):
    """Invariants on the REAL, committed captured_output.txt -- never a
    hardcoded count, always derived."""
