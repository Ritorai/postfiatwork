...
#
# The record `echo '{...NaN...}' | python3 wallet_reconcile.py - ;
# echo "exit=$?"` deliberately exercises the CLI's documented "read from
# stdin via -" code path, so unlike dup-detector's record there is no
# single-process rewrite that tests the same thing. `; echo "exit=$?"` is
# written INTO the command itself (this directory's own convention -- no
# outer harness trailer follows it, same shape as payload-validator/
# capture.sh and queue-auditor/capture.sh, whose header comments explain
# in full why only the execution wrapper -- not the recorded command text
# -- needs to change). This record's first pipe stage is a literal
# `echo`, which cannot realistically fail; it is fixed here anyway for
...
HEADER='echo '"'"'{"opening_balance":"0","closing_balance":"0","events":[{"event_id":"e1","type":"reward","amount":NaN,"at":"2026-01-01T00:00:00Z"}]}'"'"' | python3 wallet_reconcile.py - ; echo "exit=$?"'
