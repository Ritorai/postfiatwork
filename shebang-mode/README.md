...
| unwritable `-o` | exit **2** |
| **this repository** | **150 × `SM002`, exit 1** — see below |
...
Run against this checkout, the tool reports **150 × `SM002_SHEBANG_WITHOUT_EXEC`
and exits 1**. That is a real result, printed in `selfscan_output.txt`
rather than hidden:

```
counts   {"checked": 589, "executable": 0, "skipped_binary": 0,
          "skipped_excluded": 0, "skipped_not_a_regular_file": 0,
          "skipped_unmerged": 0, "tracked": 589, "with_shebang": 150}
by_code  {"SM002_SHEBANG_WITHOUT_EXEC": 150}
```

`git ls-files -s | awk '{print $1}' | sort | uniq -c` returns `589 100644`:
**no file in this repository is tracked as executable**, while 150 carry a
...
Five of those 150 are this directory's own `shebangmode.py`,
...
**The 150 are not repaired in this commit, and that is a limitation of
this commit rather than a judgement that they are fine.** Repairing them
means changing 150 index modes. A mode change cannot be expressed through
...
plus a push. Committing a checker that reports 150 real mismatches and
...
documents — should strip the 150 shebangs instead. This tool deliberately
